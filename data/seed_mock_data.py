...
print("Constructing schema structures from data/schema.sql...")
with open("data/schema.sql", "r", encoding="utf-8") as schema_file:
    cursor.executescript(schema_file.read())

# Clear records to guarantee a clean environment on reruns
...
# 4. Local Pharmacy Personnel Directory
pharmacists = [
...
# 5. Inventory Setup (Creating Stock-Outs and Surpluses for the Demo)
inventory_data = [
    ('IN-AP-VSP-PND', 'Paracetamol Tabs', 450, 5000, 450.0),    # Deficit at Pendurthi
    ('IN-AP-VSP-BHM', 'Paracetamol Tabs', 18000, 4000, 180.0),  # Surplus at Bheemili
    ('IN-AP-VZM-GJN', 'Paracetamol Tabs', 5500, 3000, 210.0),
    ('IN-AP-SKL-RUR', 'Anti-Venom Vials', 3, 40, 6.5),          # Deficit at Srikakulam Rural
    ('IN-AP-SKL-PLS', 'Anti-Venom Vials', 150, 30, 2.0)         # Surplus at Palasa
]
cursor.executemany("INSERT INTO inventory VALUES (?, ?, ?, ?, ?)", inventory_data)

# 6. Patient Traffic Logs
triage_data = [
...
# 7. Bed and Syndromic Anomaly Statistics
node_ops = [
...
conn.commit()
conn.close()
print("🎉 Success! data/smart_health.db contains complete production records.")
